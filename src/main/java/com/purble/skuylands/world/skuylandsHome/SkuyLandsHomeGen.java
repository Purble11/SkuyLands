...
public class SkuyLandsHomeGen implements IChunkGenerator {

	protected static final IBlockState MAIN_BLOCK = BlockInit.LEAED_VOID.getDefaultState();
	...
	public SkuyLandsHomeGen(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn){
		this.world = world;
		this.mapFeaturesEnabled = mapFeaturesEnabled;
		this.spawnPoint = spawn;
		this.rand = new Random(seed);
		
		spawnableCreatureList.add(new SpawnListEntry(EntitySheep.class, 16, 1, 2));
		spawnableCreatureList.add(new SpawnListEntry(EntityCow.class, 13, 1, 2));
		spawnableCreatureList.add(new SpawnListEntry(EntityChicken.class, 15, 1, 2));
	...
	public Chunk generateChunk(int x, int z) {
		...
		for (int i = 0; i < abyte.length; ++i) {
			abyte[i] = (byte)Biome.getIdForBiome(BiomeInit.SKUYLANDS);
		}
	...
	public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_) {
		...
		for (int i = -12; i <= 12; ++i) {
			for (int j = -12; j <= 12; ++j) {
				...
				if (k * k + l * l > 4096L && SkuyLandsHomeGen.islandNoise.getValue((double)k, (double)l) < -0.8999999761581421D) {
